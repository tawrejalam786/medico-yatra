...
export default function MBBSLeadCapture({ data }) {
  ...
  const mobileField = formFields.find(
    (field) => field.name === "mobile"
  );

  const cityField = formFields.find(
    (field) => field.name === "city"
  );

  const classStatusField = formFields.find(
    (field) => field.name === "classStatus"
  );

  const countryField = formFields.find(
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      id="mobile"
                      name="mobile"
                      type={mobileField?.type || "tel"}
                      inputMode="numeric"
                      placeholder={mobileField?.placeholder}
                      value={formData.mobile}
                      onChange={handleChange}
                      required={mobileField?.required}
                      className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3.5 text-xs text-[#071A49] outline-none transition placeholder:text-slate-400 focus:border-[#0263CC] focus:ring-4 focus:ring-[#0263CC]/5 sm:text-sm"
                    />
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      required={classStatusField?.required}
                    ...
                    >
                      ...
                      <option value="">
                        {classStatusField?.placeholder}
                      </option>

                      {(classStatusField?.options || []).map(
                        (option) => (
                          <option
                            key={option.value}
                            value={option.value}
