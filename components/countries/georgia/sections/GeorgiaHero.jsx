...
import { ArrowRight, BadgeCheck, BookOpenCheck, Globe2, ShieldCheck, Stethoscope } from "lucide-react";
import { PrimaryButton } from "../shared/Buttons";

const miniCards = [
  { icon: BookOpenCheck, label: "English-medium", value: "International programmes" },
  { icon: Globe2, label: "European setting", value: "Tbilisi lifestyle" },
  { icon: ShieldCheck, label: "Student friendly", value: "Safety-first planning" },
];

export default function GeorgiaHero() {
  return (
    <section className="relative overflow-hidden bg-[radial-gradient(circle_at_14%_12%,rgba(2,167,187,.16),transparent_24%),radial-gradient(circle_at_88%_18%,rgba(2,99,204,.14),transparent_26%),linear-gradient(180deg,#F8FCFF_0%,#FFFFFF_100%)] pt-8 sm:pt-12 lg:pt-22">
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[.92fr_1.08fr] lg:gap-12">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65, ease: [0.22,1,0.36,1] }}>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#0263CC]/10 bg-white px-3 py-2 text-[11px] font-black uppercase tracking-[0.16em] text-[#0263CC] shadow-sm">
              <span className="h-2 w-2 rounded-full bg-[#02A7BB]" /> Medico Yatra · Country Guide · Georgia
            </div>

            <h1 className="mt-5 max-w-3xl text-[40px] font-black leading-[1.02] tracking-[-0.055em] text-slate-950 sm:text-5xl lg:text-[64px]">
              Study MBBS in Georgia — <span className="bg-gradient-to-r from-[#0263CC] to-[#02A7BB] bg-clip-text text-transparent">a European setting, a planned career.</span>
            </h1>

            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg sm:leading-8">
              English-medium medical education, a modern student environment and a licensing pathway that should be planned from the start — not after graduation.
            </p>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs font-bold text-slate-500">
              <span className="inline-flex items-center gap-1.5"><BadgeCheck className="h-4 w-4 text-[#02A7BB]" /> NMC verification first</span>
          ...
          <motion.div className="relative" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .75, delay: .08, ease: [0.22,1,0.36,1] }}>
            <div className="relative mx-auto max-w-[680px]">
              <div className="relative aspect-[1.18/1] overflow-hidden rounded-[34px] bg-slate-100 shadow-[0_32px_90px_rgba(15,23,42,.14)] sm:rounded-[42px]">
                <img src="/images/countries/georgia/georgia-hero.webp" alt="Indian medical student in Georgia" className="h-full w-full object-cover" />
              ...
              <div className="absolute -left-2 top-8 hidden w-[185px] overflow-hidden rounded-[24px] border-4 border-white bg-white shadow-[0_20px_55px_rgba(15,23,42,.15)] sm:block lg:-left-12">
                <img src="/images/countries/georgia/georgia-city.webp" alt="Tbilisi city" className="h-28 w-full object-cover" />
                <div className="p-3">
                  <p className="text-[10px] font-black uppercase tracking-[.12em] text-[#02A7BB]">Picture the destination</p>
                  <p className="mt-1 text-sm font-black text-slate-900">Tbilisi · modern & walkable</p>
                </div>
              </div>
            </div>
          </motion.div>
