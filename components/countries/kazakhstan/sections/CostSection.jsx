    ...
    <section className="bg-white py-16 sm:py-20 lg:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[.78fr_1.22fr] lg:items-start">
          <Reveal>
            <SectionTitle eyebrow="Illustrative cost" title="Kazakhstan's biggest advantage is affordability — compare the total cost." text="The supplied brief uses placeholders rather than fixed figures, so the UI keeps costs clearly illustrative and verification-first." />
            ...
            <div className="mt-7 overflow-hidden rounded-[30px] bg-gradient-to-br from-[#0263CC] via-[#0878D7] to-[#02A7BB] p-6 text-white sm:p-7">
              ...
              <p className="mt-2 text-4xl font-black tracking-[-.045em]">5–6 years</p>
              <p className="mt-2 text-sm text-white/75">Indicative total: ₹[X–Y] lakh · tuition + living · among the lowest total costs for NMC-eligible MBBS.</p>
            ...
            <div className="mt-4 rounded-[24px] bg-[linear-gradient(135deg,#FFF4D8,#FFF1E7)] p-5 text-[#89500C]">
              <div className="flex gap-3"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0" /><p className="text-sm leading-6"><strong>Illustrative only.</strong> Compare total cost against NMC status, university quality and your FMGE preparation plan — not tuition alone.</p></div>
            </div>
          </Reveal>
          ...
          <Reveal delay={.08}>
            <div className="rounded-[32px] border border-slate-100 bg-[#F8FBFF] p-4 shadow-[0_18px_55px_rgba(15,23,42,.05)] sm:p-6">
              ...
              <a href="#lead" className="mt-5 inline-flex min-h-[50px] w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 text-sm font-black text-white transition hover:bg-[#0263CC]">Get a Personalised Kazakhstan Cost Estimate <ArrowRight className="h-4 w-4" /></a>
            </div>
          </Reveal>
        </div>
      </div>
